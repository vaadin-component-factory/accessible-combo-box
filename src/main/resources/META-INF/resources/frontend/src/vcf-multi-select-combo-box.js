 ...
 */
import './vcf-multi-select-combo-box-chip.js';
import './vcf-multi-select-combo-box-container.js';
import './vcf-multi-select-combo-box-internal.js';
...
/**
 * `<vcf-multi-select-combo-box>` is a web component that wraps `<vcf-combo-box>` and extends
 * its functionality to allow selecting multiple items, in addition to basic features.
 *
 * ```html
 * <vcf-multi-select-combo-box id="comboBox"></vcf-multi-select-combo-box>
 * ```
 *
 * ```js
 ...
 * ### Internal components
 *
 * In addition to `<vcf-multi-select-combo-box>` itself, the following internal
 * components are themable:
 *
 * - `<vcf-multi-select-combo-box-overlay>` - has the same API as `<vaadin-overlay>`.
 * - `<vcf-multi-select-combo-box-item>` - has the same API as `<vaadin-item>`.
 * - `<vcf-multi-select-combo-box-container>` - has the same API as `<vaadin-input-container>`.
 *
 * Note: the `theme` attribute value set on `<vcf-multi-select-combo-box>` is
...
class MultiSelectComboBox extends ResizeMixin(InputControlMixin(ThemableMixin(ElementMixin(PolymerElement)))) {
  static get is() {
    return 'vcf-multi-select-combo-box';
  }
...
:host([invalid]) {
  ...
  :host([readonly][has-value]) [part='toggle-button'] {
    ...
    cursor: var(--lumo-clickable-cursor);
      ...
      <div class="vcf-multi-select-combo-box-container">
        <div part="label">
          <slot name="label"></slot>
          <span part="required-indicator" aria-hidden="true" on-click="focus"></span>
        </div>

        <vcf-multi-select-combo-box-internal
          id="comboBox"
        ...
        >
          <vcf-multi-select-combo-box-container
            part="input-field"
            readonly="[[readonly]]"
            disabled="[[disabled]]"
            invalid="[[invalid]]"
  ...
  static get properties() {
    return {
       ...
       *
       * - `root` The `<vcf-multi-select-combo-box-item>` internal container DOM element.
       * - `comboBox` The reference to the `<vcf-multi-select-combo-box>` element.
  ...
  ready() {
    ...
    this._observer = new FlattenedNodesObserver(this, info => {
      this.$.comboBox._setTemplateFromNodes(info.addedNodes);
    });

    this.$.comboBox._itemTemplate = this._itemTemplate;
//    processTemplates(this);
  ...
  /** @private */
  __createChip(item) {
    const chip = document.createElement('vcf-multi-select-combo-box-chip');
  ...
  _ensureTemplatized() {
    this.$.comboBox._ensureTemplatized();
    this._TemplateClass = this.$.comboBox._TemplateClass;
  }
